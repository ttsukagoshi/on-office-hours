...
function initialTrigger() {
  ...
  console.info(
    `[initialTrigger] outOfOfficeHours: ${outOfOfficeHours}\nnowTime: ${nowTime}\nOFFICE_HOURS.start: ${OFFICE_HOURS.start}\nOFFICE_HOURS.end: ${OFFICE_HOURS.end}`
  ); // log
  if (
    !outOfOfficeHours &&
...
function resetTriggers() {
  ...
  try {
    ...
    if (
      ...
      console.info(
        `[resetTriggers] User Property: ${up.getProperty(
          UP_KEY_DATE
        )} -> todayString: ${todayString}; scriptStatus: ${scriptStatus}`
      ); // log
    ...
    console.info(
      `[resetTriggers] ${UP_KEY_DATE}: ${todayString}; scriptStatus: ${scriptStatus}`
    ); // log
  } catch (error) {
    // メールでエラー通知
...
function createOfficeHourCheckTrigger() {
  ...
  try {
    ...
    console.info(
      `[createOfficeHourCheckTrigger] scriptStatus: ${scriptStatus}`
    ); //log
  }
}
...
function officeHourCheck() {
  ...
  try {
    if (!officeOpenStatus || officeOpenStatus === 'CLOSED') {
      // 現在オフィスが閉まっている場合
      console.info(
        `[officeHourCheck] Office is closed.\n\nofficeOpenStatus: ${officeOpenStatus}`
      ); //log
...
function parseOfficeHourTrigger(officeHourString) {
  console.info(
    `[parseOfficeHourTrigger] Initiating with officeHourString: ${officeHourString}...`
  ); // log
  ...
  console.info(
    `[parseOfficeHourTrigger] Returning triggerTime: ${triggerTime}`
  ); // log
  return triggerTime;
}
