...
function initialTrigger() {
  ...
  ScriptApp.newTrigger('resetTriggers')
    ...
    .everyDays(1)
    .create();
  console.log('[initialTrigger] Complete. Current user properties:\n\n' + JSON.stringify(up.getProperties())); // log
}
...
function resetTriggers() {
  ...
  try {
    ...
    if (todayString !== up.getProperty(UP_KEY_DATE) || up.getProperty(UP_KEY_SCRIPT_STATUS) !== 'RUNNING') {
      console.log('[resetTriggers] User Property: ' + up.getProperty(UP_KEY_DATE) + '-> todayString: ' + todayString + '; scriptStatus: ' + scriptStatus); // log
    ...
    console.log('[resetTriggers] ' + UP_KEY_DATE + ': ' + todayString + '; scriptStatus: ' + scriptStatus); // log
  } catch (error) {
    // メールでエラー通知
...
function officeHourCheck() {
  ...
  try {
    if (!officeOpenStatus || officeOpenStatus === 'CLOSED') {
      ...
      // 現在オフィスが開いている場合
      console.log('[officeHourCheck] Office is open.\n\nofficeOpenStatus: ' + officeOpenStatus); // log
      if (now > OFFICE_HOURS.end) {
        officeOpenStatus = 'CLOSED';
        console.log('[officeHourCheck] Office is now closed.\n\nnow: ' + now + '\nOFFICE_HOURS start: ' + OFFICE_HOURS.start + ', end: ' + OFFICE_HOURS.end + '\nofficeOpenStatus: ' + officeOpenStatus); // log
    ...
    scriptStatus = 'RUNNING';
    console.log('[officeHourCheck] Final officeOpenStatus: ' + officeOpenStatus); // log
  } catch (error) {
    // メールでエラー通知
    var myEmail = Session.getActiveUser().getEmail();
...
function parseOfficeHourTrigger(officeHourString) {
  console.log('[parseOfficeHourTrigger] Initiating with officeHourString: ' + officeHourString + '...'); // log
