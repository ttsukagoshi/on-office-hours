...
function initialTrigger() {
  ...
  ScriptApp.newTrigger('resetTriggers')
    ...
    .everyDays(1)
    .create();
  Logger.log('[initialTrigger] Complete. Current user properties:\n\n' + JSON.stringify(up.getProperties())); // log
}
...
function resetTriggers() {
  ...
  try {
    ...
    if (todayString !== up.getProperty(UP_KEY_DATE) || up.getProperty(UP_KEY_SCRIPT_STATUS) !== 'RUNNING') {
      Logger.log('[resetTriggers] User Property: ' + up.getProperty(UP_KEY_DATE) + '-> todayString: ' + todayString + '; scriptStatus: ' + scriptStatus); // log
    ...
    Logger.log('[resetTriggers] ' + UP_KEY_DATE + ': ' + todayString + '; scriptStatus: ' + scriptStatus); // log
  } catch (error) {
    // メールでエラー通知
      ...
      if (now >= OFFICE_HOURS.start) {
        ...
        ScriptApp.getProjectTriggers().forEach(function (trigger) {
          if (trigger.getHandlerFunction() === 'officeHourCheck') {
            ScriptApp.deleteTrigger(trigger);
            Logger.log('[officeHourCheck] Trigger for officeHourCheck is deleted.'); // log
          }
        });
      }
    } else if (officeOpenStatus === 'OPEN') {
      // 現在オフィスが開いている場合
      Logger.log('[officeHourCheck] Office is open.\n\nofficeOpenStatus: ' + officeOpenStatus); // log
      if (now > OFFICE_HOURS.end) {
        officeOpenStatus = 'CLOSED';
        Logger.log('[officeHourCheck] Office is now closed.\n\nnow: ' + now + '\nOFFICE_HOURS start: ' + OFFICE_HOURS.start + ', end: ' + OFFICE_HOURS.end + '\nofficeOpenStatus: ' + officeOpenStatus); // log
...
function parseOfficeHourTrigger(officeHourString) {
  Logger.log('[parseOfficeHourTrigger] Initiating with officeHourString: ' + officeHourString + '...'); // log
